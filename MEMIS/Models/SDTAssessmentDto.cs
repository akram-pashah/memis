...
{
  public class SDTAssessmentDto:IValidatableObject
  {
    ...
    public bool ShouldValidateDDSCComments()
    {
      return (DDCSAction == "Rejected");
    }
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
      ...
      {
        yield return new ValidationResult("Justification is required when Averge Days is greater than Target.", new[] { nameof(Justification) });
      }
      if (ShouldValidateDDSCComments() && string.IsNullOrWhiteSpace(DDCSComment))
      {
        yield return new ValidationResult("Comment is mandatory when rejected.", new[] { nameof(DDCSComment) });
      }
